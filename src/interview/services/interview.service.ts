...
import { RESUME_ANALYSIS_SYSTEM_MESSAGE, RESUME_QUIZ_PROMPT2 } from "../prompts/resume.quiz.prompts";
...
import { ConsumptionRecord, ConsumptionRecordDocument, ConsumptionStatus, ConsumptionType } from "../schemas/consumption-record.schema";
import { Model, Types } from "mongoose";
import { InjectModel } from "@nestjs/mongoose";
import { ResumeQuizResult, ResumeQuizResultDocument, ResumeQuizResultSchema } from '../schemas/interview-quiz-result.schema';
...
export class InterviewService {
  [x: string]: any;
    ...
    // 执行简历押题
    private async executeResumeQuiz(userId: string, dto: ResumeQuizDto, progressSubject?: Subject<ProgressEvent>) {
        ...
        if(dto.requestId) {
          ...
          if (existingRecord) {
            if (existingRecord.status === ConsumptionStatus.SUCCESS) {
              this.logger.log(`重复请求，返回已有结果：requestId = ${dto.requestId}`);
  ...
  private emitProgress(subject: Subject<ProgressEvent> | undefined, progress: number, label: string, stage?: 'prepare' | 'generating' | 'saving' | 'done') {
  ...
  private async getRemainingCount(userId: string, type: 'resume' | 'special' | 'behavior'): Promise<number> {
