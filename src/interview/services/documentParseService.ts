    ...
    **/
        ...
        try {
            ...
            switch (fileType) {
                case 'PDF':
                    ...
                    console.log("🚀 ~ DocumentParseService ~ parseDocumentFromUrl ~ text:", text)
                    break;
                case 'DOCX':
    ...
    private async parseDocx(buffer: Buffer): Promise<string>{
        try {
            ...
            if (!result.value || result.value.trim().length === 0) {
                throw new BadRequestException('DOCX 文件无法提取内容，请检查是否为空文件');
            }

            if (result.messages && result.messages.length > 0) {
                this.logger.warn(`DOCX警告：${result.messages.map((m) => m.message).join(', ')}`);
            }
    ...
    private async parsePDF(buffer: Buffer): Promise<string> {
        try {
            ...
            this.logger.log(`pdf解析成功，页数=${data.numpages}, 长度=${data.text.length}`);
    ...
    private async downloadFile(url: string): Promise<Buffer> {
        try {
            ...
            const stream = await this.minioService.getFileStream('resume', 'sunday-resume.pdf');
            const chunks: Buffer[] = [];
    ...
    private validateUrl(url: string) {
        ...
        if (!fileType) {
            throw new BadRequestException(`不支持的文件格式。支持的格式：${Object.values(this.SUPPORTED_TYPES).flat().join(', ')}`);
        }
    ...
    cleanText(text: string): string {
        ...
        return (
            ...
            .replace(/ {2,}/g, ' ')
            //6.去除页眉页脚常见标记
            .replace(/第\s*\d+\s*页/g,'').replace(/Page\s+\d+/gi,'')
            //7.整体trim
            .trim()
        );
    }
