 ...
 * 仅押题部分的格式说明
 */
export const FORMAT_INSTRUCTIONS_QUESTIONS_ONLY1 = `
请严格按照以下JSON格式返回结果：
 ...
 * 生成题目
 */
export const RESUME_QUIZ_PROMPT_QUESTIONS_ONLY1 = `# 角色设定
...
# 任务目标
基于候选人简历和目标岗位要求，生成高质量、个性化、针对性强的面试问题清单。
...
3. **真实性**：模拟真实面试场景，避免教科书式提问
4. **差异化**：避免通用问题，体现岗位特色
每个问题需包含：question, category, difficulty, keywords, reasoning
...
1. 如果简历中缺少某些JD要求的技能，也要出1道相关问题（标记为hard）
2. 每个category至少包含不同difficulty的题目
3. **所有分析必须基于简历和JD的实际内容**，不要臆测
4. ⚠️ **JSON格式要求**：
   - 所有文本内容必须是单行字符串
   - 不要在JSON字符串中直接换行
   - 如需换行效果，段落之间用空格分隔即可
